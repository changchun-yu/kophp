...
/**
 * The Ko_Header class provides an Object-Orientated interface
...
class Header extends \ArrayObject {
    ...
    /**
     * Constructor method for [Ko_Header]. Uses the standard constructor
     * of the parent `ArrayObject` class.
     *
     *     $header_object = new Header(array('x-powered-by' => 'Ko 3.1.x', 'expires' => '...'));
     *
    ...
    public function accepts_at_quality($type, $explicit = FALSE)
    {
        ...
        if (strpos($type, '/') === FALSE) {
            $mime = Ko::$config->load('mimes.' . $type);
     ...
     * @since   3.2.0
     */
    public function send_headers(\ko\Response $response = NULL, $replace = FALSE, $callback = NULL)
    {
        $protocol = $response->protocol();
        $status = $response->status();

        // Create the response header
        $processed_headers = array($protocol . ' ' . $status . ' ' . \ko\Response::$messages[$status]);
...
//            $processed_headers[] = Text::ucfirst($header) . ': ' . $value;
        ...
        if (!isset($headers['content-type'])) {
            $processed_headers[] = 'Content-Type: ' . Ko::$content_type . '; charset=' . Ko::$charset;
        }

        if (Ko::$expose AND ! isset($headers['x-powered-by'])) {
            $processed_headers[] = 'X-Powered-By: ' . Ko::version();
        }
