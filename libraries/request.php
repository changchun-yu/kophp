...
use ko\http\Header;
use ko\request\client\Internal;
 ...
 */
class Request implements \ko\http\Request {
    ...
    public static function detect_uri()
    {
        if (!empty($_SERVER['PATH_INFO'])) {
            ...
            if (Ko::$index_file AND strpos($uri, Ko::$index_file) === 0) {
                // Remove the index file from the URI
                $uri = (string) substr($uri, strlen(Ko::$index_file));
            }
        }
    ...
    /**
     * @var  Ko_HTTP_Header  headers to sent as part of the request
     */
    protected $_header;
    ...
    public function execute()
    {
        ...
        if (!$this->_client instanceof Client) {
            throw new Exception('Unable to execute :uri without a Ko_Request_Client', array(
        ':uri' => $this->_uri,
            ));
        }
    ...
    /**
     * Returns whether this request is the initial request Ko received.
     ...
     *  - Body
     *
     *  If there are variables set to the `Ko_Request::$_post`
    ...
    public function render()
    {
        if (!$post = $this->post()) {
            ...
            $this->body($body)
                    ->headers('content-type', 'application/x-www-form-urlencoded; charset=' . Ko::$charset);
        }

        // Set the content length
        $this->headers('content-length', (string) $this->content_length());

        // If Ko expose, set the user-agent
        if (Ko::$expose) {
            $this->headers('user-agent', Ko::version());
        }
